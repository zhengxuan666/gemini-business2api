<template>
  <div class="space-y-8">
    <section
      v-if="isLoading"
      class="rounded-3xl border border-border bg-card p-6 text-sm text-muted-foreground"
    >
      正在加载设置...
    </section>
              ...
              <div class="mt-4 space-y-3">
                <div class="grid grid-cols-2 items-center gap-x-6 gap-y-2">
                  ...
                  <div class="flex items-center justify-end gap-2">
                    ...
                    <HelpTip
                      text="无头模式适用于服务器环境（如 Docker）。若注册/刷新失败，建议关闭。"
                    />
                  </div>
                </div>
                <div
                ...
                >
                  <span>浏览器引擎</span>
                  <HelpTip
                    text="UC: 支持无头/有头，但可能失败。DP: 支持无头/有头，更稳定，推荐使用。"
                  />
                </div>
                <SelectMenu
                ...
                >
                  <span>过期刷新窗口（小时）</span>
                  <HelpTip
                    text="当账号距离过期小于等于该值时，会触发自动登录刷新（更新 cookie/session）。"
                  />
                </div>
                <input
          ...
          <div class="space-y-4">
            <div class="rounded-2xl border border-border bg-card p-4">
              ...
              <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
                ...
                <input
                  v-model.number="
                    localSettings.retry.rate_limit_cooldown_seconds
                  "
                  type="number"
                  min="0"
                  class="col-span-2 rounded-2xl border border-input bg-background px-3 py-2"
                ...
                <div
                  class="col-span-2 flex items-center justify-between gap-2 text-xs text-muted-foreground"
                >
                  <span>自动刷新账号间隔（秒，0禁用）</span>
                  <HelpTip
                    text="仅在数据库存储启用时生效：用于检测账号配置变化并重载列表，不会刷新 cookie。文件存储模式不会触发。"
                  />
                </div>
                <input
                  v-model.number="
                    localSettings.retry.auto_refresh_accounts_seconds
                  "
                  type="number"
                  min="0"
                  max="600"
          ...
          <div class="space-y-4">
            <div class="rounded-2xl border border-border bg-card p-4">
              ...
              <div class="mt-4 space-y-3">
                ...
                <Checkbox
                  v-model="localSettings.image_generation.images_api_enabled"
                >
                  启用 OpenAI Images API 端点
                </Checkbox>
                <label class="block text-xs text-muted-foreground"
                  >输出格式</label
                >
...
watch(settings, (value) => {
  ...
  next.image_generation = next.image_generation || {
    ...
    images_api_enabled: false,
  };
  next.image_generation.output_format ||= "base64";
  next.image_generation.images_api_enabled =
    next.image_generation.images_api_enabled ?? false;
  ...
  next.basic.refresh_window_hours = Number.isFinite(
    next.basic.refresh_window_hours
  )
    ? next.basic.refresh_window_hours
    : 1;
  next.basic.register_default_count = Number.isFinite(
    next.basic.register_default_count
  )
    ? next.basic.register_default_count
    : 1;
  next.basic.register_domain =
    typeof next.basic.register_domain === "string"
      ? next.basic.register_domain
      : "";
  next.basic.duckmail_api_key =
    typeof next.basic.duckmail_api_key === "string"
      ? next.basic.duckmail_api_key
      : "";
  next.retry = next.retry || {};
  next.retry.auto_refresh_accounts_seconds = Number.isFinite(
